...
#
# =============================================================================
# Script:  sub-timestamp.py
# Purpose: MQTT latency SUBSCRIBER for the ToD testbed. Subscribes to "set/<ms>"
#          and, for each timestamp message published by pub-timestamp.py,
#          computes the inter-arrival gap (ms) between consecutive messages.
#          Gaps above the 110 ms threshold are flagged as latency/jitter events
#          (i.e. degraded delivery, useful for spotting fault-injection impact).
#
# Usage:   ./sub-timestamp.py <ms_name>
#          e.g. ./sub-timestamp.py speed  -> subscribes to "set/speed"
#
# Config:  BROKERIP   env var (required) - MQTT broker host
#          BROKERPORT env var (required) - MQTT broker port
# Output:  Prints sent/received times, the delta, and flags deltas > 110 ms.
#
# NOTE (review): the delta measures the gap BETWEEN received messages (arrival
#   jitter), not true one-way latency, since publisher and subscriber clocks may
#   differ. The 110 ms threshold assumes the 100 ms publish interval.
# =============================================================================
